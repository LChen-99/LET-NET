...
int main(int argc, char** argv) {
    ...
    const float mean_vals_in[1] = {0};
    const float norm_vals_in[1] = {1.0/255.0};
    ...
    while (true) {
        ...
        in.substract_mean_normalize(mean_vals_in, norm_vals_in);
        ...
        ex.extract("desc", out2);
        std::cout << "w:" << out2.w << ",h:" << out2.h << std::endl;
        std::cout << "channel" << out2.c << std::endl;
...
//        memcpy((uchar*)score.data, out1.data, sizeof(float) * out1.w * out1.h);
        ...
        std::cout << desc.channels() << std::endl;
        cv::imshow("desc", desc);
        cv::waitKey(-1);
